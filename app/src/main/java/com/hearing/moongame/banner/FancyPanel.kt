package com.hearing.moongame.banner
...
import com.hearing.moongame.utils.hide
import com.hearing.moongame.utils.show
...
class FancyPanel @JvmOverloads constructor(
    ...
    private val marginL: Float = SizeUtil.dp2px(12f).toFloat(),
    private val marginT: Float = SizeUtil.dp2px(12f).toFloat(),
    private val marginR: Float = SizeUtil.dp2px(12f).toFloat(),
    private val marginB: Float = SizeUtil.dp2px(12f).toFloat()
) : ConstraintLayout(context, attrs, defStyleAttr) {

    // ---------------------------------- 控件 ---------------------------------
    private lateinit var bigPanel: View
    private lateinit var littlePanel: View
    private lateinit var closeView: View
    private lateinit var foldView: View
    private lateinit var transView: View
    private lateinit var scrollTextView: ScrollTextView

    // 小面板的尺寸
    private val littlePanelSize = SizeUtil.dp2px(45f).toFloat()

    // 小面板吸附的边缘距离
    private val littlePanelPadding = SizeUtil.dp2px(12f).toFloat()
    ...
    init {
        LayoutInflater.from(context).inflate(R.layout.layout_multi_panel, this, true)?.apply {
            bigPanel = this.findViewById(R.id.bigPanel)
            littlePanel = this.findViewById(R.id.littlePanel)
            foldView = this.findViewById(R.id.foldView)
            transView = this.findViewById(R.id.transView)
            closeView = this.findViewById(R.id.closeView)
            scrollTextView = this.findViewById(R.id.scrollTextView)
        }
        correctBigBorder()
    ...
    fun setPanelContent(content: String) {
        ...
        scrollTextView.highLightColor = Color.YELLOW
    ...
    private fun onLittlePanelTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_UP -> {
                ...
                // click
                if (moveDistance <= SizeUtil.dp2px(2f)) {
                    unfold()
                }
            }
        }
