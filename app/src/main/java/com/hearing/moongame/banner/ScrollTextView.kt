...
class ScrollTextView @JvmOverloads constructor(
    ...
    // ------------------------------- 内部参数 -------------------------------
    private var scrollSpeed = SizeUtil.dp2px(45f).toFloat() / FREQUENCY // speed default 45dp/s
    private var refreshRate = (1000 / FREQUENCY).toLong() // 默认 16ms 刷新一次
    private val textLineSpace = SizeUtil.dp2px(10f).toFloat() // 行间距
    private val defaultTextSize = SizeUtil.dp2px(20f).toFloat() // 默认字体大小
    ...
    fun setContentSize(size: Int) {
        val pxSize = SizeUtil.dp2px(size.toFloat()).toFloat()
    ...
    fun setSpeed(speed: Float) {
        this.scrollSpeed = SizeUtil.dp2px(speed).toFloat() / FREQUENCY
